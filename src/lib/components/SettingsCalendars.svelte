  ...
  {#each CALENDARS as c (c.id)}
    <SettingRow
      ...
      explain={present(c.entity)
        ? `${c.purpose} · ${c.entity}${c.source === "ical" ? " · subscribed" : c.source === "builtin" ? " · generated by HA" : ""}`
        : `Needs ${c.entity} in Home Assistant. ${
            c.source === "ical"
              ? "Subscribe to the feed rather than retyping the dates."
              : c.source === "builtin"
                ? "Add the Holiday integration and pick South Africa — it generates the dates itself."
                : "Add a Local Calendar with this name."
          }`}
