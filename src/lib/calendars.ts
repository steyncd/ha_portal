...
export type Cal = {
  ...
  entity: string;
  /** "builtin" is an integration that generates the dates itself — the Holiday
   *  integration for ZA public holidays. Not a subscription and not hand-typed,
   *  so it needs neither a URL nor maintenance. */
  source: "local" | "ical" | "builtin";
...
export const CALENDARS: Cal[] = [
  {
    ...
    // A local calendar, not a subscription: created 2026-08-10 because no ICS
    // URL for the school's term dates was available. Term dates go in by hand
    // once a year, which is less work than finding a feed that stays correct.
    source: "local",
    ...
    effects: [
      ...
      { text: "Morning briefing moves earlier on a school day", target: "input_datetime.daily_briefing_time" },
    ],
  },
  {
    id: "away",
  ...
  },
  {
    ...
    entity: "calendar.south_africa",
    // HA's built-in Holiday integration, country ZA (added 2026-08-10). Better
    // than an ICS subscription: nothing to renew, and it cannot go stale or
    // 404 in a year's time.
    source: "builtin",
    purpose: "Generated by Home Assistant. A public holiday is not a Saturday and not a school day.",
    effects: [
