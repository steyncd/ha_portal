<script lang="ts">
  ...
  const templates = $derived(
    Object.keys(ha.entities)
      .filter((id) => id.startsWith("input_text.msg_"))
      .map((id) => ({ id, name: ha.name(id).replace(/^Msg:\s*/, ""), value: ha.state(id) ?? "" }))
  ...
  async function refresh() {
    ...
    // one row per uid, earliest upcoming occurrence
  ...
  function parseCfg(r: Reminder) {
    try { return (r.description ?? "").trim().startsWith("{") ? JSON.parse(r.description!) : {}; } catch { return {}; }
  }
  function destLabels(r: Reminder) {
    const c = parseCfg(r);
    const out: string[] = [];
  ...
  function newForm() {
    ...
    return {
      ...
      when: toLocalInput(d), repeat: "none" as "none" | "daily" | "weekly" | "monthly",
      days: [] as string[],
      announce: true, speaker: "media_player.study_speaker",
  ...
  function openEdit(r: Reminder) {
    const c = parseCfg(r);
    const d = start(r) ? new Date(start(r)!) : new Date();
    f = {
      ...
      announce: !!c.announce, speaker: c.announce || "media_player.study_speaker",
  ...
  const canSave = $derived(f.title.trim() && (f.useTemplate ? f.template : true) && (f.announce || recipients.length > 0));

  function rrule() {
    if (f.repeat === "daily") return "FREQ=DAILY";
    if (f.repeat === "monthly") return `FREQ=MONTHLY;BYMONTHDAY=${new Date(f.when).getDate()}`;
    if (f.repeat === "weekly") {
      const days = f.days.length ? f.days.join(",") : WEEKDAYS[(new Date(f.when).getDay() + 6) % 7][0];
  ...
  function payload() {
    return JSON.stringify({
      msg: f.useTemplate ? "" : f.msg.trim(),
      template: f.useTemplate ? f.template : "",
      announce: f.announce ? f.speaker : "",
      wa: recipients,
    });
  }
  async function save() {
    ...
    building = false; editingUid = null;
    setTimeout(refresh, 400);
  }
  async function del(r: Reminder) {
    if (!r.uid) return;
    await ha.deleteReminder(r.uid, r.recurrence_id);
    toast.show("Reminder deleted");
    reminders = reminders.filter((x) => x.uid !== r.uid);
  }
  function sendTest() {
    ha.dispatchReminder({ message: f.useTemplate ? "" : f.msg, template: f.useTemplate ? f.template : "", announce: f.announce ? f.speaker : "", wa: recipients });
  ...
  async function doPreview() {
    const body = f.useTemplate
      ? (f.template ? `states(${JSON.stringify(f.template)})` : `''`)
      : JSON.stringify(f.msg);
    preview = await ha.renderTemplate(`{% from 'messages.jinja' import render %}{{ render(${body}) }}`);
  }

  // ---- template editor ----
  let editId = $state("");
  let editVal = $state("");
  let tplPreview = $state("");
  function pickTemplate(id: string) { editId = id; editVal = templates.find((t) => t.id === id)?.value ?? ""; tplPreview = ""; }
  async function previewTpl() { tplPreview = await ha.renderTemplate(`{% from 'messages.jinja' import render %}{{ render(${JSON.stringify(editVal)}) }}`); }
  function saveTpl() { if (!editId) return; ha.setText(editId, editVal); toast.show("Template saved"); }
</script>

<div class="col">
  <div class="card head">
    <div>
      <div class="lb">Reminders</div>
      <div class="sub">Announce on speakers + WhatsApp, once-off or on repeat.</div>
    </div>
    <div class="hact">
  ...
  {#if building}
    <div class="card pad builder">
      ...
      {#if f.repeat === "weekly"}
        <div class="days">
          {#each WEEKDAYS as [code, lbl]}
            <button class="day" class:on={f.days.includes(code)} onclick={() => (f.days = f.days.includes(code) ? f.days.filter((d) => d !== code) : [...f.days, code])}>{lbl}</button>
          {/each}
        </div>
      {/if}
  ...
  <div class="card pad">
    ...
    {:else if reminders.length === 0}
      <div class="empty">No reminders yet. Tap “New reminder” to add one.</div>
    {:else}
      <div class="rlist">
        {#each reminders as r}
          <div class="rrow">
            <div class="rmain">
              <div class="rtitle">{r.summary}</div>
              <div class="rmeta">{whenLabel(r)} · <span class="rep">{repeatLabel(r)}</span></div>
            </div>
            <div class="rdest">{#each destLabels(r) as d}<span class="chip">{d}</span>{/each}</div>
            <div class="rbtns">
              <button class="rb" onclick={() => openEdit(r)}>Edit</button>
              <button class="rb del" onclick={() => del(r)}>Delete</button>
            </div>
          </div>
        {/each}
      </div>
  ...
  <!-- template management -->
  <div class="card pad">
    <div class="lb" style="margin-bottom:12px">Message templates</div>
    <div class="tsel">
      {#each templates as t}
        <button class="tchip" class:on={editId === t.id} onclick={() => pickTemplate(t.id)}>{t.name}</button>
      {/each}
      {#if templates.length === 0}<span class="sub">No msg_* templates found.</span>{/if}
    </div>
    {#if editId}
      <textarea class="ted" bind:value={editVal} rows="3"></textarea>
      <div class="prev"><button class="pbtn" onclick={previewTpl}>Preview</button>{#if tplPreview}<span class="pv">“{tplPreview}”</span>{/if}</div>
      <div class="actions"><div class="spacer"></div><button class="save" onclick={saveTpl}>Save template</button></div>
      <div class="hint">Placeholders like {"{greeting} {battery} {tank} {alarm}"} are filled in live when sent.</div>
    {/if}
  </div>
</div>
...
<style>
  ...
  .new { padding: 11px 16px; border-radius: 12px; background: var(--grad); color: #0b1017; font-size: 13px; font-weight: 800; }
  ...
  .rb:hover { background: rgba(255, 255, 255, 0.12); color: var(--text); }
  .rb.del { color: #fecdd6; background: color-mix(in srgb, var(--error) 16%, transparent); }
  .tsel { display: flex; gap: 8px; flex-wrap: wrap; margin-bottom: 12px; }
  ...
  .ted { margin-bottom: 10px; }
  .hint { font-size: 11.5px; color: var(--muted-2); margin-top: 10px; }
</style>
